...
class LinearNakayamaAlgebra(pathAlgebra.PathAlgebra):
    ...
    def isGentle(self):
        """Whether the algebra is gentle: every relation exactly two arrows long.

        A gentle algebra is a monomial algebra whose ideal is generated by paths
        of **length two**, with at most two arrows in and two out of each vertex
        and the two compatibility conditions on arrow pairs.  On a line quiver
        each vertex has at most one arrow in and one out, so everything but the
        length of the relations is automatic -- and a relation of three or more
        arrows puts the algebra outside the class, however gentle-looking the
        quiver is.  It is still a *string* algebra either way.

        Nothing in the pipeline turns on this; it is here because the derived
        invariants of gentle algebras were a candidate for separating classes the
        quipu theorem does not reach, and F-019 is why that route is closed.
        """
        return all(arrows in (0, 2) for arrows in self.relLengths)
