"""Older ways of generating quipus, kept for cross-checking `quipuForms`.

`quipuForms.allQuipusOfOrder` enumerates the P^(m)_(k) parameters and
canonicalises them; `generateAllQuipus` here enumerates non-isomorphic trees and
filters by degree instead.  The two agree on the counts for orders 4 to 12
(research E-013), which is worth having as an independent check, since one works
from the notation and the other from the graph.

The rest is superseded or unfinished -- `generateAllHeightOneQuipus` does not
run -- and none of it is used by the package.
"""
...
def generateAllHeightOneQuipus( mainStringLength, mainStringLengthStart = 1 ):
    ...
    for i in range(mainStringLengthStart,mainStringLength):
        ...
        vertexPowerset = paths.powerset(range(1,i))
        j = 0
        while j < 2**(i-1):
