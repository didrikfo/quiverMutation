...
# -- the gentle LNAs are all one class ------------------------------------

@pytest.mark.parametrize("length", [4, 5, 6, 7, 8, 9, 10])
def test_every_gentle_lna_is_the_hereditary_one(length):
    """A gentle LNA is derived equivalent to the path algebra of A_n, always.

    Gentle means the ideal is generated by paths of length two, which on a line
    quiver is exactly "every relation has two arrows".  Two facts then collide:
    such an algebra always has almost separate relations (the condition
    n_{i+1} >= n_i + l_i - 1 reads n_{i+1} >= n_i + 1 when every l_i is 2, and
    the starts are distinct), and operation 2 of `cor:EquivNakayamaAlgebras`
    drops a relation of two arrows without changing the class.  Drop them all and
    nothing is left.

    So the gentle LNAs of length n are the 2^(n-2) subsets of the relation
    positions, and every one of them is in the single class P^(0)_(0,n-1).  That
    is why the derived invariants of gentle algebras cannot separate any class
    this project cares about -- F-019.
    """
    gentle = [a for a in nk.LinearNakayamaAlgebra.allOfLength(length) if a.isGentle()]
    assert len(gentle) == 2 ** (length - 2)
    hereditary = qf.formatQuipu(qf.quipuParameters(
        qf.underlyingGraph(nk.LinearNakayamaAlgebra(length, [0] * (length - 2)))))
    for algebra in gentle:
        assert algebra.hasAlmostSeparateRelations()
        assert algebra.withoutShortRelations().relLengths == [0] * (length - 2)
        assert algebra.quipuName() == hereditary
    assert hereditary == "P^(0)_(0,{0})".format(length - 1)


def test_one_longer_relation_leaves_the_gentle_class():
    """The boundary: three arrows is not gentle, and lands elsewhere."""
    algebra = nk.LinearNakayamaAlgebra(6, [3, 0, 0, 0])
    assert not algebra.isGentle()
    assert algebra.quipuName() != "P^(0)_(0,5)"
